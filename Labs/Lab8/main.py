...
#-------------------------------------------------------------------------------
#
# X = np.random.uniform(0, 360, 1000)
# y = np.sin(np.deg2rad(X))
#
# import keras
# model = keras.models.Sequential()
# model.add(keras.layers.Dense(100, activation='tanh', input_dim=1))
# model.add(keras.layers.Dense(100, activation='tanh'))
# model.add(keras.layers.Dense(1, activation='linear'))
# model.compile(optimizer='adam', loss=keras.losses.mean_absolute_error, metrics=[])
# model.fit(X, y, epochs=300, batch_size=32, verbose=2)
#
# #x1 = np.random.randn(100)*360
# y1 = model.predict(X)
# # print np.sin(np.deg2rad(x))
#
# plt.scatter(X, y, c='red', s=4)
# plt.scatter(X, y1, c='blue', s=4)
# plt.show()
